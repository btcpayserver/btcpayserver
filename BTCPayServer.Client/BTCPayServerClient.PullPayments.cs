...
public partial class BTCPayServerClient
{
    ...
    {
        var path = storeId is null
            ? $"api/v1/pull-payments/{HttpUtility.UrlEncode(pullPaymentId)}"
            : $"api/v1/stores/{HttpUtility.UrlEncode(storeId)}/pull-payments/{HttpUtility.UrlEncode(pullPaymentId)}";
        await SendHttpRequest(path, null, HttpMethod.Delete, cancellationToken);
    }

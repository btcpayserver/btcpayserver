...
public class SubscriberData : BaseEntityData
{
    ...
    public decimal GetCredit()
        => Credits.FirstOrDefault(c => c.Currency == Plan.Currency)?.Amount ?? 0m;

    public decimal MissingCredit()
        => Math.Max(0m, Plan.Price - GetCredit());
    ...
    public int PaymentReminderDays { get; set; } = 7;
    ...
    [NotMapped]
    public bool CanStartNextPlan => this is
    {
    ...
    public PhaseTypes GetExpectedPhase(DateTimeOffset time)
        => this switch
        {
            ...
            { Plan: { RecurringType: PlanData.RecurringInterval.Forever }, PaidAmount: not null } => PhaseTypes.Normal,
    ...
    public static void OnModelCreating(ModelBuilder builder, DatabaseFacade databaseFacade)
    {
        ...
        b.Property(x => x.PaymentReminderDays).HasDefaultValue(7);
    ...
    public DateTimeOffset? GetReminderDate()
    {
        ...
        if (date is null)
            return null;
        return date - TimeSpan.FromDays(PaymentReminderDays);
    }

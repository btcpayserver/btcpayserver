...
namespace BTCPayServer.Tests
{
    ...
    public class CheckoutUITests : UnitTestBase
    {
        ...
        public async Task CanConfigureCheckoutWithLNURL()
        {
            ...
            Assert.Equal(3, s.Driver.FindElements(By.CssSelector(".payment-method")).Count);
            Assert.Contains("Bitcoin", s.Driver.FindElement(By.CssSelector(".payment-method.active")).Text);
            Assert.Contains("Lightning", s.Driver.FindElement(By.CssSelector(".payment-method:nth-child(2)")).Text);
            Assert.Contains("LNURL", s.Driver.FindElement(By.CssSelector(".payment-method:nth-child(3)")).Text);
            ...
            // Switch to Lightning for the topup
            ...
            TestUtils.Eventually(() =>
            {
                ...
                address = s.Driver.FindElement(By.CssSelector("#Lightning_BTC-CHAIN .truncate-center")).GetAttribute("data-text");
            ...
            // Switch to LNURL
            s.Driver.FindElement(By.CssSelector(".payment-method:nth-child(3)")).Click();
            TestUtils.Eventually(() =>
            {
                payUrl = s.Driver.FindElement(By.Id("PayInWallet")).GetAttribute("href");
            ...
            Assert.Contains($"?lightning={copyAddressLightning}", payUrl);
            Assert.DoesNotContain("amount=", payUrl);
            Assert.StartsWith("bcrt", copyAddressOnchain);
            Assert.StartsWith("lnbcrt1", copyAddressLightning);
        ...
        [Fact(Timeout = TestTimeout)]
        [Trait("Lightning", "Lightning")]
        public async Task CanConfigureCheckoutWithoutLNURL()
        {
            using var s = CreateSeleniumTester();
            s.Server.ActivateLightning();
            await s.StartAsync();

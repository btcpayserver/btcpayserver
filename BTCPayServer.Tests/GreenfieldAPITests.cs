    ...
    public class GreenfieldAPITests
    {
        ...
        public async Task SpecificCanModifyStoreCantCreateNewStore()
        {
            using (var tester = ServerTester.Create())
            {
                ...
                await this.AssertHttpError(403,
                    async () => await restricted.CreateStore(new CreateStoreRequest() {Name = "store2"}));
        ...
        public async Task CanCreateUsersViaAPI()
        {
            using (var tester = ServerTester.Create(newDb: true))
            {
                ...
                {
                    LockSubscription = false, DisableNonAdminCreateUserApi = true
                });
                await AssertHttpError(403,
                    async () =>
                        await unauthClient.CreateUser(
            ...
            using (var tester = ServerTester.Create())
            {
                ...
                await this.AssertAPIError("overdraft",
                    async () => await unauthenticated.CreatePayout(pps[0].Id,
                        new CreatePayoutRequest()
                        {
                            Destination = destination, Amount = 0.00001m, PaymentMethod = "BTC"
                        }));
        ...
        public async Task StoresControllerTests()
        {
            using (var tester = ServerTester.Create())
            {
                ...
                var storeEntity = await ctx.UserStore.SingleAsync(u =>
                    u.ApplicationUserId == user.UserId && u.StoreDataId == newStore.Id);
                storeEntity.Role = "Guest";
                await ctx.SaveChangesAsync();
                await AssertHttpError(403,
                    async () => await client.UpdateStore(newStore.Id, new UpdateStoreRequest() {Name = "B"}));
            }
        }
        ...
        public async Task CanUseWebhooks()
        {
            ...
            var hook = await clientProfile.CreateWebhook(user.StoreId,
                new CreateStoreWebhookRequest() {Url = fakeServer.ServerUri.AbsoluteUri, AutomaticRedelivery = false});
            ...
            fakeServer.Done();
            hook = await clientProfile.UpdateWebhook(user.StoreId, hook.Id,
                new UpdateStoreWebhookRequest() {Url = hook.Url, Secret = "lol", AutomaticRedelivery = false});
            ...
            {
                await clientProfile.UpdateWebhook(user.StoreId, "lol",
                    new UpdateStoreWebhookRequest() {Url = hook.Url});
            });
        ...
        public async Task PaymentControllerTests()
        {
            using (var tester = ServerTester.Create())
            {
                ...
                var invoiceId = Assert.IsType<string>(Assert.IsType<OkObjectResult>(await user
                    .GetController<PaymentRequestController>()
                ...
                await TestUtils.EventuallyAsync(async () =>
                {
                    ...
                    Assert.Equal(PaymentRequestData.PaymentRequestStatus.Completed,
                        (await client.GetPaymentRequest(user.StoreId, paymentTestPaymentRequest.Id)).Status);
                });
            }
        }
        ...
        public async Task InvoiceLegacyTests()
        {
            using (var tester = ServerTester.Create())
            {
                ...
                var oldInvoice = await oldBitpay.CreateInvoiceAsync(new Invoice()
                {
                    ...
                    BuyerAddress1 = "blah",
                    Buyer = new Buyer() {Address2 = "blah2"},
                    ItemCode = "code",
                    ItemDesc = "desc",
                    OrderId = "orderId",
                    PosData = "posData"
                ...
                Logs.Tester.LogInformation(
                    "Let's hack the Bitpay created invoice to be just like before this update. (Invoice V1)");
                var invoiceV1 = "{\r\n  \"version\": 1,\r\n  \"id\": \"" + oldInvoice.Id + "\",\r\n  \"storeId\": \"" +
                                user.StoreId +
                ...
                Logs.Tester.LogInformation(
                    "Now, let's create an invoice with the new API but with the same metadata as Bitpay");
                newInvoice.Metadata.Add("lol", "lol");
                newInvoice = await client.CreateInvoice(user.StoreId,
                    new CreateInvoiceRequest()
                    {
                        Metadata = newInvoice.Metadata, Amount = 1000.19392922m, Currency = "BTC"
                    });
        ...
        public async Task InvoiceTests()
        {
            using (var tester = ServerTester.Create())
            {
                ...
                await AssertValidationError(
                    new[]
                    {
                        nameof(CreateInvoiceRequest.Currency), nameof(CreateInvoiceRequest.Amount),
                        $"{nameof(CreateInvoiceRequest.Checkout)}.{nameof(CreateInvoiceRequest.Checkout.PaymentTolerance)}",
                        $"{nameof(CreateInvoiceRequest.Checkout)}.{nameof(CreateInvoiceRequest.Checkout.PaymentMethods)}[0]"
                    }, async () =>
                    {
                        await client.CreateInvoice(user.StoreId,
                            new CreateInvoiceRequest()
                            {
                                Amount = -1,
                                Checkout = new CreateInvoiceRequest.CheckoutOptions()
                                {
                                    PaymentTolerance = -2, PaymentMethods = new[] {"jasaas_sdsad"}
                                }
                            });
                    });
                ...
                await client.MarkInvoiceStatus(user.StoreId, newInvoice.Id,
                    new MarkInvoiceStatusRequest() {Status = InvoiceStatus.Settled});
                newInvoice = await client.CreateInvoice(user.StoreId,
                    new CreateInvoiceRequest() {Currency = "USD", Amount = 1});
                await client.MarkInvoiceStatus(user.StoreId, newInvoice.Id,
                ...
                foreach (var marked in new[] {InvoiceStatus.Settled, InvoiceStatus.Invalid})
                {
                    ...
                    await client.MarkInvoiceStatus(user.StoreId, inv.Id,
                        new MarkInvoiceStatusRequest() {Status = marked});
                    ...
                    if (marked == InvoiceStatus.Invalid)
                    {
                        ...
                        var evt = user.AssertHasWebhookEvent<WebhookInvoiceInvalidEvent>(
                ...
                newInvoice = await client.CreateInvoice(user.StoreId,
                    new CreateInvoiceRequest()
                    {
                        ...
                        Checkout = new CreateInvoiceRequest.CheckoutOptions()
                        {
                            DefaultLanguage = "it-it ", RedirectURL = "http://toto.com/lol"
                        }
                    });
        ...
        public async Task CanUseLightningAPI()
        {
            using (var tester = ServerTester.Create())
            {
                ...
                var merchantClient =
                    await merchant.CreateClient($"{Policies.CanUseLightningNodeInStore}:{merchant.StoreId}");
                ...
                var err = await Assert.ThrowsAsync<HttpRequestException>(async () =>
                    await client.GetLightningNodeChannels("BTC"));
                Assert.Contains("503", err.Message);
                // Not permission for the store!
                err = await Assert.ThrowsAsync<HttpRequestException>(async () =>
                    await client.GetLightningNodeChannels(user.StoreId, "BTC"));
                Assert.Contains("403", err.Message);
                var invoiceData = await client.CreateLightningInvoice("BTC",
                ...
                invoiceData = await client.CreateLightningInvoice(user.StoreId, "BTC",
                    new CreateLightningInvoiceRequest()
                    {
                        Amount = LightMoney.Satoshis(1000),
                        Description = "lol",
                ...
                await client.PayLightningInvoice(user.StoreId, "BTC",
                    new PayLightningInvoiceRequest() {BOLT11 = merchantInvoice.BOLT11});
                await Assert.ThrowsAsync<GreenFieldValidationException>(async () =>
                    await client.PayLightningInvoice(user.StoreId, "BTC",
                        new PayLightningInvoiceRequest() {BOLT11 = "lol"}));

                var validationErr = await Assert.ThrowsAsync<GreenFieldValidationException>(async () =>
                    await client.CreateLightningInvoice(user.StoreId, "BTC",
                        new CreateLightningInvoiceRequest()
                        {
                            Amount = -1, Expiry = TimeSpan.FromSeconds(-1), Description = null
                        }));
        ...
        public async Task OnChainPaymentMethodAPITests()
        {
            ...
            await AssertHttpError(403, async () =>
            {
                await viewOnlyClient.UpdateStoreOnChainPaymentMethod(store.Id, "BTC",
                    new OnChainPaymentMethodData() { });
            });
            ...
            var firstAddress = xpriv.Derive(KeyPath.Parse("0/0")).Neuter().GetPublicKey()
                .GetAddress(ScriptPubKeyType.Segwit, Network.RegTest).ToString();
            await AssertHttpError(404, async () =>
            {
                await client.PreviewStoreOnChainPaymentMethodAddresses(store.Id, "BTC");
            });

            Assert.Equal(firstAddress, (await viewOnlyClient.PreviewProposedStoreOnChainPaymentMethodAddresses(store.Id,
                "BTC",
            ...
            method = await client.UpdateStoreOnChainPaymentMethod(store.Id, "BTC",
                new OnChainPaymentMethodData()
                {
                    ...
                    Label = "lol",
                    AccountKeyPath = RootedKeyPath.Parse("01020304/1/2/3")
                });
            ...
            Assert.Equal(firstAddress,
                (await viewOnlyClient.PreviewStoreOnChainPaymentMethodAddresses(store.Id, "BTC")).Addresses.First()
                .Address);
            await AssertHttpError(403, async () =>
            {
        ...
        public async Task LightningNetworkPaymentMethodAPITests()
        {
            ...
            var admin2Client =
                await admin2.CreateClient(Policies.CanModifyStoreSettings, Policies.CanModifyServerSettings);
            ...
            await AssertHttpError(403, async () =>
            {
                await viewOnlyClient.UpdateStoreLightningNetworkPaymentMethod(store.Id, "BTC",
            ...
            {
                "type=clightning;server=tcp://127.0.0.1", "type=clightning;server=tcp://test",
                "type=clightning;server=tcp://test.lan", "type=clightning;server=tcp://test.local",
                "type=clightning;server=tcp://192.168.1.2", "type=clightning;server=unix://8.8.8.8",
                "type=clightning;server=unix://[::1]", "type=clightning;server=unix://[0:0:0:0:0:0:0:1]",
            })
            {
                var ex = await AssertValidationError(new[] {"ConnectionString"}, async () =>
                {
                    await adminClient.UpdateStoreLightningNetworkPaymentMethod(store.Id, "BTC",
                        new LightningNetworkPaymentMethodData()
                        {
                            ConnectionString = forbidden, CryptoCode = "BTC", Enabled = true
                        });
                });
                ...
                await admin2Client.UpdateStoreLightningNetworkPaymentMethod(admin2.StoreId, "BTC",
                    new LightningNetworkPaymentMethodData()
                    {
                        ConnectionString = forbidden, CryptoCode = "BTC", Enabled = true
                    });
            ...
            await adminClient.UpdateStoreLightningNetworkPaymentMethod(store.Id, "BTC",
                new LightningNetworkPaymentMethodData()
                {
                    ConnectionString = "type=clightning;server=tcp://8.8.8.8", CryptoCode = "BTC", Enabled = true
                });
            ...
            var settings =
                (await tester.PayTester.GetService<SettingsRepository>().GetSettingAsync<PoliciesSettings>()) ??
        ...
        public async Task WalletAPITests()
        {
            ...
            Assert.NotNull(tx);
            Assert.Contains(tx.Outputs,
                txout => txout.IsTo(nodeAddress) && txout.Value.ToDecimal(MoneyUnit.BTC) == 0.001m);
            ...
            createTxRequest.SelectedInputs = new List<OutPoint>() {utxo.Outpoint};
            tx = await client.CreateOnChainTransactionButDoNotBroadcast(walletId.StoreId, walletId.CryptoCode,
                createTxRequest, tester.ExplorerClient.Network.NBitcoinNetwork);
            ...
            tx = await client.CreateOnChainTransactionButDoNotBroadcast(walletId.StoreId, walletId.CryptoCode,
                createTxRequest, tester.ExplorerClient.Network.NBitcoinNetwork);
            Assert.Contains(tx.Outputs,
                txout => txout.Value.GetValue(tester.NetworkProvider.GetNetwork<BTCPayNetwork>("BTC")) == 0.0001m);
            ...
            await AssertHttpError(403, async () =>
            {
                await viewOnlyClient.GetOnChainWalletTransaction(walletId.StoreId, walletId.CryptoCode,
                    txdata.TransactionHash.ToString());
            });
            await client.GetOnChainWalletTransaction(walletId.StoreId, walletId.CryptoCode,
                txdata.TransactionHash.ToString());
            ...
            Assert.Contains(
                await client.ShowOnChainWalletTransactions(walletId.StoreId, walletId.CryptoCode),
                data => data.TransactionHash == txdata.TransactionHash);

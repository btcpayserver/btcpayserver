...
public class SubscriptionTests(ITestOutputHelper testOutputHelper) : UnitTestBase(testOutputHelper)
{
    ...
    class OfferingPMO(PlaywrightTester s)
    {
        ...
        public Task AddPlan()
            => s.Page.GetByRole(AriaRole.Button, new() { Name = "Add Plan" }).ClickAsync();

        public async Task NewSubscriber(string planName, string email, bool hasTrial, bool mine = false)
        {
    ...
    class AddEditPlanPMO(PlaywrightTester tester)
    {
        ...
        public async Task Save()
        {
            ...
            if (OptimisticActivation is true)
                await s.Page.GetByRole(AriaRole.Checkbox, new() { Name = "Optimistic activation" }).CheckAsync();
            if (OptimisticActivation is false)
                await s.Page.GetByRole(AriaRole.Checkbox, new() { Name = "Optimistic activation" }).UncheckAsync();
            await s.ClickPagePrimary();
            await s.FindAlertMessage();
        }

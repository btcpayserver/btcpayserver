...
public partial class VaultBridgeUI
{
    public abstract class HWIController : VaultController
    {
        ...
        protected abstract Task Run(VaultBridgeUI ui, HwiClient hwi, HwiDeviceClient device, HDFingerprint fingerprint, BTCPayNetwork network, CancellationToken cancellationToken);
        protected override async Task Run(VaultBridgeUI ui, VaultClient vaultClient, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                ui.ShowFeedback(VaultElement.Feedback.StateValue.Loading, ui.StringLocalizer["Fetching device..."]);
                var version = await hwi.GetVersionAsync(cancellationToken);
                if (version.Major < 2)
                {
                    ui.ShowFeedback(VaultElement.Feedback.StateValue.Failed,
                        ui.ViewLocalizer[
                ...
                if (finished == timeout)
                    ui.ShowFeedback(VaultElement.Feedback.StateValue.Loading, ui.StringLocalizer["Please, enter the passphrase on the device."]);
                var entries = await gettingEntries;
                var deviceEntry = entries.FirstOrDefault();

                if (deviceEntry is null)
                {
                    ui.ShowFeedback(VaultElement.Feedback.StateValue.Failed, ui.StringLocalizer["No device connected."]);
                    ui.ShowRetry();
                    return;
                }
                ...
                ui.ShowFeedback(VaultElement.Feedback.StateValue.Success, ui.StringLocalizer["Device found: {0}", device.GetNiceModelName()]);
                ...
                if (deviceEntry is { Code: HwiErrorCode.DeviceNotReady })
                {
                    ...
                    else if (deviceEntry.NeedsPinSent is true)
                    {
                        ...
                        var pinElement = new VaultElement.PinInput(ui);
                        var pin = await pinElement.GetPin();
                        if (!await device.SendPinAsync(pin, cancellationToken))
                        {
                            ui.ShowFeedback(VaultElement.Feedback.StateValue.Failed, ui.StringLocalizer["Incorrect pin code."]);
                            ui.ShowRetry();
                            return;
                        }

                        ui.ShowFeedback(VaultElement.Feedback.StateValue.Success, ui.StringLocalizer["Pin code verified."]);
                        dirtyDevice = true;
                    }
                    else if (deviceEntry.NeedsPassphraseSent is true)
                    {
                        var passwordEl = new VaultElement.Passphrase(ui);
                        device.Password = await passwordEl.GetPassword();
                    }
                }
                else if (deviceEntry is { Code: HwiErrorCode.DeviceNotInitialized })
                {
                    ui.ShowFeedback(VaultElement.Feedback.StateValue.Failed, ui.StringLocalizer["The device has not been initialized."]);
                    ui.ShowRetry();
                    return;
                }
                ...
                {
                    ui.ShowFeedback(VaultElement.Feedback.StateValue.Loading, ui.StringLocalizer["Fetching wallet's fingerprint."]);
                    ...
                    ui.ShowFeedback(VaultElement.Feedback.StateValue.Success, ui.StringLocalizer["Wallet's fingerprint fetched."]);
                }
            ...
            catch (HwiException e)
            {
                ...
                ui.ShowFeedback(VaultElement.Feedback.StateValue.Failed, message);
                ui.ShowRetry();
            }
        }
    }
    ...
    public class SignHWIController : HWIController
    {
        ...
        protected override async Task Run(VaultBridgeUI ui, HwiClient hwi, HwiDeviceClient device, HDFingerprint fingerprint, BTCPayNetwork network, CancellationToken cancellationToken)
        {
            ...
            if (store is null || derivationSettings is null)
                return;
            ui.ShowFeedback(VaultElement.Feedback.StateValue.Loading, ui.StringLocalizer["Checking if this device can sign the transaction..."]);
            ...
            if (signableInputs.Length > 0)
            {
                ...
                if (derivationSettings.IsMultiSigOnServer)
                {
                    ...
                    if (alreadySigned)
                    {
                        ui.ShowFeedback(VaultElement.Feedback.StateValue.Failed, ui.StringLocalizer["This device already signed PSBT."]);
                        ui.ShowRetry();
                        return;
                    }
                }
            ...
            ui.ShowFeedback(VaultElement.Feedback.StateValue.Loading,
            ...
            ui.ShowFeedback(VaultElement.Feedback.StateValue.Loading, ui.StringLocalizer["Transaction signed successfully, proceeding to review..."]);
    ...
    public class GetXPubController : HWIController
    {
        protected override async Task Run(VaultBridgeUI ui, HwiClient hwi, HwiDeviceClient device, HDFingerprint fingerprint, BTCPayNetwork network, CancellationToken cancellationToken)
        {
            var xpubSelect = new VaultElement.XPubSelect(ui, network.NBitcoinNetwork);
            var xpubInfo = await xpubSelect.GetXPubSelect();
            var scriptPubKeyTypeType = xpubInfo.ToScriptPubKeyType();
            ui.ShowFeedback(VaultElement.Feedback.StateValue.Loading, ui.StringLocalizer["Fetching public keys..."]);
            ...
            ui.ShowFeedback(VaultElement.Feedback.StateValue.Success, ui.StringLocalizer["Public keys successfully fetched."]);
            ...
            var verif = new VaultElement.VerifyAddress(ui, device, keyPath.Derive(firstDepositPath), firstDepositAddr, xpubInfo.ToScriptPubKeyType());
            ...
            ui.ShowFeedback(VaultElement.Feedback.StateValue.Loading, ui.StringLocalizer["Saving..."]);
            ...
            settings.AccountKeySettings[0].RootFingerprint = device.Fingerprint;
            ...
            var fp = device.Fingerprint is { } f ? $" ({f})" : "";
            settings.Label = $"{device.GetNiceModelName()}{fp}";

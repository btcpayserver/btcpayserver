    ...
    public partial class InvoiceController
    {
        ...
        public async Task<IActionResult> Invoice(string invoiceId)
        {
            ...
            var model = new InvoiceDetailsModel()
            {
                ...
                StoreLink = Url.Action(nameof(StoresController.Payment), "Stores", new { storeId = store.Id }),
        ...
        public async Task<IActionResult> CreateInvoice(CreateInvoiceModel model, CancellationToken cancellationToken)
        {
            ...
            if (!store.GetSupportedPaymentMethods(_NetworkProvider).Any())
            {
                TempData.SetStatusMessageModel(new StatusMessageModel()
                {
                    ...
                    Html = $"To create an invoice, you need to <a href='{Url.Action(nameof(StoresController.Payment), "Stores", new { storeId = store.Id })}' class='alert-link'>set up your wallet</a> first",
                    AllowDismiss = false
                });
                return View(model);
            }

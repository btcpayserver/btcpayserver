    ...
    public partial class UIInvoiceController
    {
        ...
        public async Task<IActionResult> Refund([FromServices] IEnumerable<IPayoutHandler> payoutHandlers, string invoiceId, CancellationToken cancellationToken)
        {
            ...
            foreach (var pmi in pmis.ToList())
            {
                ...
                {
                    pmis.Add(new PaymentMethodId(lh.Network.CryptoCode, LightningPaymentType.Instance));
                }
            }
        ...
        public async Task<IActionResult> Refund(string invoiceId, RefundModel model, CancellationToken cancellationToken)
        {
            ...
            if (!pms.TryGetValue(paymentMethodId, out var paymentMethod))
            {
                ...
                {
                    pms.TryGetValue(new PaymentMethodId(lnh.Network.CryptoCode, PaymentTypes.LNURLPay), out paymentMethod);
                }
            }
        ...
        private async Task<PaymentModel?> GetInvoiceModel(string invoiceId, PaymentMethodId? paymentMethodId, string? lang)
        {
            ...
            var btcId = new PaymentMethodId("BTC", PaymentTypes.BTCLike);
            var lnurlId = new PaymentMethodId("BTC", PaymentTypes.LNURLPay);
            var lnId = new PaymentMethodId("BTC", PaymentTypes.LightningLike);
            ...
            if (paymentMethodId is null)
            {
                ...
                if (paymentMethodId is null)
                {
                    var defaultBTC = new PaymentMethodId(_NetworkProvider.DefaultNetwork.CryptoCode, PaymentTypes.BTCLike);
                    var defaultLNURLPay  = new PaymentMethodId(_NetworkProvider.DefaultNetwork.CryptoCode, PaymentTypes.LNURLPay);
            ...
            string GetPaymentMethodName(PaymentMethodId paymentMethodId)
            {
                ...
                return extension?.DisplayName ?? paymentMethodId.ToPrettyString();
            }
...
#pragma warning restore CS0618 // Type or member is obsolete
                ...
                AvailableCryptos = invoice.GetPaymentPrompts()
                                          .Select(kv =>
                                          {
                                              ...
                                              return new PaymentModel.AvailableCrypto
                                              {
                                                  ...
                                                  IsLightning = handler is LNURLPayPaymentHandler or LightningLikePaymentHandler,
        ...
        private SelectList GetPaymentMethodsSelectList(StoreData store)
        {
            ...
            return new SelectList(store.GetPaymentMethodConfigs()
                    ...
                    .Select(method => new SelectListItem(method.Key.ToPrettyString(), method.Key.ToString())),

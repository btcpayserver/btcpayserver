    ...
    public class UIPullPaymentController : Controller
    {
        ...
        public async Task<IActionResult> ViewPullPayment(string pullPaymentId)
        {
            ...
            ViewPullPaymentModel vm = new(pp, DateTimeOffset.UtcNow)
            {
                ...
                AmountFormatted = _currencyNameTable.DisplayFormatCurrency(blob.Limit, blob.Currency),
                AmountCollected = totalPaid,
                AmountCollectedFormatted = _currencyNameTable.DisplayFormatCurrency(totalPaid, blob.Currency),
                AmountDue = amountDue,
                ClaimedAmount = amountDue,
                AmountDueFormatted = _currencyNameTable.DisplayFormatCurrency(amountDue, blob.Currency),
                CurrencyData = cd,
                ...
                Payouts = payouts
                          .Select(entity => new ViewPullPaymentModel.PayoutLine
                          {
                              ...
                              AmountFormatted = _currencyNameTable.DisplayFormatCurrency(entity.Blob.Amount, blob.Currency),

...
) : Controller
{
    ...
    [HttpPost("stores/{storeId}/offerings/{offeringId}/subscribers/{customerId}/suspend")]
    [Authorize(Policy = Policies.CanModifyMembership, AuthenticationSchemes = AuthenticationSchemes.Cookie)]
    public async Task<IActionResult> SubscriberSuspend(string storeId, string offeringId, string customerId, string command )
    {
        await using var ctx = dbContextFactory.CreateContext();
        ...
        if (canSwitch)
        {
            await subsService.ToggleSuspend(sub.Id);
            var subName = sub.Customer.GetPrimaryIdentity() ?? sub.CustomerId;
            await ctx.Entry(sub).ReloadAsync();
            var message = sub.IsSuspended ?
                StringLocalizer["Subscriber is now {0} suspended", subName] :
                StringLocalizer["Subscriber is now {0} unsuspended", subName];

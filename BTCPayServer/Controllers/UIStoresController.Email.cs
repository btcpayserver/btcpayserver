...
public partial class UIStoresController
{
    ...
    public async Task<IActionResult> StoreEmails(string storeId)
    {
        ...
        {
            var emailSender = await emailSenderFactory.GetEmailSender(store.Id) as StoreEmailSender;
    ...
    public async Task<IActionResult> StoreEmailSettings(string storeId)
    {
        ...
        var fallbackSettings = await emailSenderFactory.GetEmailSender(store.Id) is StoreEmailSender { FallbackSender: not null } storeSender
    ...
    public async Task<IActionResult> StoreEmailSettings(string storeId, EmailsViewModel model, string command, [FromForm] bool useCustomSMTP = false)
    {
        ...
        model.FallbackSettings = await emailSenderFactory.GetEmailSender(store.Id) is StoreEmailSender { FallbackSender: not null } storeSender
            ? await storeSender.FallbackSender.GetEmailSettings()
            : null;
        if (useCustomSMTP)
        {

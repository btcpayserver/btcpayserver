...
public partial class UIStoresController
{
    ...
    {
        return (await repo.GetWebhookDeliveries(CurrentStore.Id, webhookId, 1)).ToList().FirstOrDefault();
    }

    [HttpGet("{storeId}/webhooks")]
    public async Task<IActionResult> Webhooks()
    {
        var webhooks = await repo.GetWebhooks(CurrentStore.Id);
    ...
    {
        var webhook = await repo.GetWebhook(CurrentStore.Id, webhookId);
        if (webhook is null)
            return NotFound();
    ...
    public async Task<IActionResult> DeleteWebhookPost(string webhookId)
    {
        ...
        await repo.DeleteWebhook(CurrentStore.Id, webhookId);
    ...
    public async Task<IActionResult> NewWebhook(string storeId, EditWebhookViewModel viewModel)
    {
        ...
        await repo.CreateWebhook(CurrentStore.Id, viewModel.CreateBlob());
    ...
    public async Task<IActionResult> ModifyWebhook(string webhookId)
    {
        ...
        var deliveries = await repo.GetWebhookDeliveries(CurrentStore.Id, webhookId, 20);
    ...
    public async Task<IActionResult> ModifyWebhook(string webhookId, EditWebhookViewModel viewModel)
    {
        ...
        await repo.UpdateWebhook(CurrentStore.Id, webhookId, viewModel.CreateBlob());
    ...
    {
        var delivery = await repo.GetWebhookDelivery(CurrentStore.Id, webhookId, deliveryId);
        if (delivery is null)
            return NotFound();

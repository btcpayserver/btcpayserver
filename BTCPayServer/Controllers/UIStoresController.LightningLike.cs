...
public partial class UIStoresController
{
    ...
    public IActionResult Lightning(string storeId, string cryptoCode)
    {
        ...
        if (vm.LightningNodeType == LightningNodeType.Internal)
        {
            ...
            // other services
            foreach ((string key, Uri value) in externalServiceOptions.Value.OtherExternalServices)
            {
    ...
    public async Task<IActionResult> SetupLightningNode(string storeId, LightningNodeViewModel vm, string command, string cryptoCode)
    {
        ...
        else
        {
            ...
            try
            {
                lightningClient = lightningClientFactoryService.Create(vm.ConnectionString, network);
            }
            catch (Exception e)
            {
        ...
        switch (command)
        {
            ...
            case "test":
                var handler = serviceProvider.GetRequiredService<LightningLikePaymentHandler>();
                try
                {
    ...
    {
        return LightningNetworkOptions.InternalLightningByCryptoCode.ContainsKey(cryptoCode.ToUpperInvariant()) && (User.IsInRole(Roles.ServerAdmin) || policiesSettings.AllowLightningInternalNodeForAll);
    }

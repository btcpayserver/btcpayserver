...
public partial class UIStoresController
{
    ...
    public async Task<IActionResult> StoreUsers(string storeId, StoreUsersViewModel vm)
    {
        ...
        var roles = await repo.GetStoreRoles(CurrentStore.Id);
        ...
        var isExistingStoreUser = isExistingUser && await repo.GetStoreUser(storeId, user!.Id) is not null;
        ...
        if (user == null)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var settings = await settingsRepository.GetSettingAsync<EmailSettings>() ?? new EmailSettings();
        ...
        var roleId = await repo.ResolveStoreRoleId(storeId, vm.Role);
        var action = isExistingUser
            ? isExistingStoreUser ? "updated" : "added"
            : "invited";
        if (await repo.AddOrUpdateStoreUser(CurrentStore.Id, user.Id, roleId))
        {
    ...
    {
        var roleId = await repo.ResolveStoreRoleId(storeId, vm.Role);
        var storeUsers = await repo.GetStoreUsers(storeId);
        ...
        else if (await repo.AddOrUpdateStoreUser(storeId, userId, roleId))
    ...
    {
        if (await repo.RemoveStoreUser(storeId, userId))
    ...
    {
        var users = await repo.GetStoreUsers(CurrentStore.Id);

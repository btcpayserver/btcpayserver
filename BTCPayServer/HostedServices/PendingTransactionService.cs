    ...
    : EventHostedServiceBase(eventAggregator, logger), IPeriodicTask
{
    ...
    public async Task<PendingTransaction?> CollectSignature(PendingTransacitonFullId id, PSBT psbt, CancellationToken cancellationToken)
    {
        await using var ctx = dbContextFactory.CreateContext();
    ...
    public record PendingTransacitonFullId(string CryptoCode, string StoreId, string Id);
    public async Task<PendingTransaction?> GetPendingTransaction(PendingTransacitonFullId id)
    {
        await using var ctx = dbContextFactory.CreateContext();
    ...
    public async Task CancelPendingTransaction(PendingTransacitonFullId id)
    {
        await using var ctx = dbContextFactory.CreateContext();
    ...
    public async Task Broadcasted(PendingTransacitonFullId id)
    {
        await using var ctx = dbContextFactory.CreateContext();
        var pt = await ctx.PendingTransactions.FirstOrDefaultAsync(p =>
            p.CryptoCode == id.CryptoCode && p.StoreId == id.StoreId && p.Id == id.Id &&

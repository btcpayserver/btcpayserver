...
{
    public DynamicDnsHostedService(IHttpClientFactory httpClientFactory, SettingsRepository settingsRepository, Logs logs)
        : base(logs)
    {
        ...
        using (var timeout = CancellationTokenSource.CreateLinkedTokenSource(CancellationToken))
        {
            ...
            foreach (var service in settings.Services)
            {
                                         ...
                                         (DateTimeOffset.UtcNow - service.LastUpdated) > Period))
                {
                    ...
                    try
                    {
                        var errorMessage = await service.SendUpdateRequest(HttpClientFactory.CreateClient(), timeout.Token);

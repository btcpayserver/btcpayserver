...
namespace BTCPayServer.Plugins.GlobalSearch
{
        ...
        : Controller
    {
            ...
            SearchContext context)
        {
            ...
            foreach (var provider in globalSearchProviders)
            {
                try
                {
                ...
                {
                    logger.LogError(ex, "Global search provider {Provider} failed", provider.GetType().FullName);
                }
            }
        }
        ...
        private async Task<StoreData> ResolveStore(string storeId, string userId, bool isServerAdmin)
        {
            ...
            return isServerAdmin
                ? await storeRepository.FindStore(storeId)
                : await storeRepository.FindStore(storeId, userId);
        }
        ...
        private async Task<string[]> GetStoreIdsWithPermission(SearchContext context, string policy)
        {
            ...
            else
            {
                ...
                {
                    context.UserStores ??= (await storeRepository.GetStoresByUserId(context.UserId)).ToArray();
                    storeIds = context.UserStores
                        .Where(data => data.HasPolicy(context.UserId, policy, permissionService))
                        .Select(data => data.Id)
        ...
        {
            var result = await authorizationService.AuthorizeAsync(User, storeId, new PolicyRequirement(policy));

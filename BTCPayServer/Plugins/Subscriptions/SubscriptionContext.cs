    ...
    private static async Task<decimal?> UpdateCredit(BalanceTransaction tx, bool force, ApplicationDbContext ctx)
    {
        ...
        var amount = await ctx.Database.GetDbConnection()
            .ExecuteScalarAsync<decimal?>($"""
                                           WITH
                                           up AS (
                                               INSERT INTO subscriptions_subscriber_credits AS c (subscriber_id, currency, amount)
                                           ...
                                           hist AS (
                                               INSERT INTO subscriptions_subscriber_credits_history(subscriber_id, currency, description, debit, credit, balance)
                                               SELECT
                                                up.subscriber_id,
                                                up.currency,
                                                @desc,

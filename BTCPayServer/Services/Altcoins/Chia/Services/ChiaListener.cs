...
namespace BTCPayServer.Services.Altcoins.Chia.Services
{
    public class ChiaListener : IHostedService
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            leases.Add(_eventAggregator.Subscribe<ChiaEvent>(OnChiaEvent));
            ...
            _ = ConnectToDaemon(cts.Token);
            _ = WorkThroughQueue(_Cts.Token);
            return Task.CompletedTask;
        }

        private async Task ConnectToDaemon(CancellationToken token)
        {
            Console.WriteLine("Connect");
            await Task.WhenAll(_ChiaRpcProvider.DaemonRpcClients.Select(async client =>
            {
                await client.Value.Connect(token);
                client.Value.BroadcastMessageReceived += OnBroadcastMessageReceived;
            }));
        ...
        private void OnBroadcastMessageReceived(object sender, Message message)
        {
            Console.WriteLine(message);
        }

        private void OnChiaEvent(ChiaEvent obj)
        {
            if (!_ChiaRpcProvider.IsAvailable(obj.CryptoCode))
            {
                return;
            }
        ...
        private async Task UpdatePaymentStates(string cryptoCode, InvoiceEntity[] invoices)
        {
            ...
            var height = await _ChiaRpcProvider.FullNodeRpcClients[cryptoCode]
                .SendCommandAsync<JsonRpcClient.NoRequestModel, GetHeightInfoResponse>(
                    "get_height_info",
                    new JsonRpcClient.NoRequestModel());
            ...
            foreach (var keyValuePair in tasks)
            {
                ...
                transactionProcessingTasks.AddRange(transactions.Select(transaction =>
                {
                    ...
                    var confirmations = height.Height - transaction.ConfirmedAtHeight;
        ...
        {
            var paymentMethodId = new PaymentMethodId(cryptoCode, ChiaPaymentType.Instance);
            var transaction = await _ChiaRpcProvider.WalletRpcClients[cryptoCode]
                .SendCommandAsync<GetTransactionRequest, GetTransactionResponse>(
                    "get_transaction",
                    new GetTransactionRequest() { TransactionId = transactionHash });


            var height = await _ChiaRpcProvider.WalletRpcClients[cryptoCode]
                .SendCommandAsync<JsonRpcClient.NoRequestModel, GetHeightInfoResponse>(
                    "get_height_info",
                    new JsonRpcClient.NoRequestModel());

            var confirmations = height.Height - transaction.Transaction.ConfirmedAtHeight;

            var paymentsToUpdate = new BlockingCollection<(PaymentEntity Payment, InvoiceEntity invoice)>();

            var address = transaction.Transaction.ToAddress;
            ...
            if (invoice != null)
            {
                await HandlePaymentData(cryptoCode,
                    ...
                    confirmations,
                    transaction.Transaction.ConfirmedAtHeight

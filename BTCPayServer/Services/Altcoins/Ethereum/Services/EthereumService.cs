...
namespace BTCPayServer.Services.Altcoins.Ethereum.Services
{
    public class EthereumService : EventHostedServiceBase
    {
        ...
        private  async Task LoopThroughChainWatchers(CancellationToken cancellationToken)
        {
            ...
            foreach (var chainId in chainIds)
            {
                try
                {
                    ...
                    var valid = await EthereumConfigController.CheckValid(_btcPayNetworkProvider.NetworkType, settings?.InvoiceId);
        ...
        protected override async Task ProcessEvent(object evt, CancellationToken cancellationToken)
        {
            ...
            {
                var valid = await EthereumConfigController.CheckValid(_btcPayNetworkProvider.NetworkType, settingsChangedEthConfig?.Settings?.InvoiceId);

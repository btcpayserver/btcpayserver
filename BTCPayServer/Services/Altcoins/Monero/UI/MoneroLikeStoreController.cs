                ...
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        ...
                        TempData.SetStatusMessageModel(new StatusMessageModel()
                        {
                            ...
                            Message = $"Remote node {viewModel.RemoteNodeAddress}:{viewModel.RemoteNodePort} added successfully."
                        });
                    }
                    catch (Exception ex)
                    {
        ...
        public class MoneroLikePaymentMethodViewModel : IValidatableObject
        {
            ...
            public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
            {
                ...
                if (UseRemoteNode == true)
                {
                    ...
                    {
                        yield return new ValidationResult("Please provide an address for the remote node.", new[] { nameof(RemoteNodeAddress) });
                    }

                    if (RemoteNodePort == null)
                    {
                        yield return new ValidationResult("Please provide a port number for the remote node.", new[] { nameof(RemoteNodePort) });
                    }
                }
            }
        }

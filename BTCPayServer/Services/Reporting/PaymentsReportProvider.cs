        ...
        return new()
        {
            ...
            Charts = 
            {
                new ()
                {
                    ...
                    Groups = { "PaymentCurrency", "PaymentType" },
    ...
    public override async Task Query(QueryContext queryContext, CancellationToken cancellation)
    {
        ...
        foreach (var invoice in invoices)
        {
            foreach (var payment in invoice.GetPayments(true))
            {
                ...
                var paymentId = payment.PaymentMethodId;
                _handlers.TryGetValue(paymentId, out var handler);
                if (handler is LightningLikePaymentHandler or LNURLPayPaymentHandler)
                {
                    ...
                    values.Add(paymentId.ToString());
                values.Add(paymentId.ToString());

                values.Add(payment.Status is PaymentStatus.Settled);
                values.Add(payment.Destination);
                values.Add(payment.Currency);
                values.Add(payment.Value);
                values.Add(payment.PaymentMethodFee);

                var prompt = invoice.GetPaymentPrompt(new PaymentMethodId("BTC", PaymentTypes.LNURLPay));
                var consumerdLightningAddress = prompt is null || handler is not LNURLPayPaymentHandler ? null : ((LNURLPayPaymentMethodDetails)handler.ParsePaymentPromptDetails(prompt.Details)).ConsumedLightningAddress;

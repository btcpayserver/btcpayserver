...
public class OnChainAutomatedTransferSenderFactory : EventHostedServiceBase, ITransferProcessorFactory
{
    ...
    public OnChainAutomatedTransferSenderFactory(EventAggregator eventAggregator,
        ILogger<OnChainAutomatedTransferSenderFactory> logger,
    ...
    public static string ProcessorName => nameof(OnChainAutomatedTransferSenderFactory);
    ...
    public string ConfigureLink(string storeId, PaymentMethodId paymentMethodId, HttpRequest request)
    {
        return _linkGenerator.GetUriByAction("Configure", 
            "UIOnChainAutomatedTransferProcessors",new
            {
                storeId,
    ...
    public async Task<IHostedService> ConstructProcessor(TransferProcessorData settings)
    {
        ...
        return ActivatorUtilities.CreateInstance<OnChainTransferSender>(_serviceProvider, settings);
    }
}

...
const posCommon = {
    ...
    beforeMount() {
        ...
        this.items.forEach(item => {
            ...
            // This is a workaround to get ASP.NET to generate the proper url (resolved with an appended version)
            if (item.hasImage) {
                item.image = document.getElementById(item.id + "_image").getAttribute('src');
            }
        });
    },
    mounted () {

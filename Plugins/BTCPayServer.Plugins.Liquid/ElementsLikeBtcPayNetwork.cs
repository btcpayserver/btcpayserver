...
public class ElementsBTCPayNetwork : BTCPayNetwork
{
    ...
    public override decimal GetValue(IMoney m)
    {
        if (m is AssetMoney assetMoney)
        {
            return assetMoney.AssetId == AssetId ? MoneyExtensions.Convert(assetMoney.Quantity, Divisibility) : 0;
        }
    ...
    {
        return NetworkCryptoCode == CryptoCode ? null : AssetId.ToString();
    }
}
